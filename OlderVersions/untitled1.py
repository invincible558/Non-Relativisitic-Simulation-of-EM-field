...
def simulate_relativistic(force):
    ...
    c=canvas(background=color.black,width=1500,height=500)
    obj=sphere(canvas=c,pos=vector(0,0,0),radius=10,make_trail=True,texture={'file':textures.stucco})
    make_axes(100)
    c2=canvas(background=color.black,width=1500,height=500)
    obj2=sphere(canvas=c2,pos=vector(0,0,0),radius=0.5,make_trail=True,texture={'file':textures.stucco})
    V2=V
    make_axes(100)
    dt=0.0001 
    time=0
    ...
    B=modifyB(obj.pos.x,obj.pos.y,obj.pos.z,time)
    while (time<10 and V.x*2+V.y2+V.z*2 < 0.99 ):
        rate(8000)
        time+=dt
        gamma=modifygamma(V)
        dtau=gamma*dt
...
def create_menu():
    ...
    def openinputfield(sys):
        ...
        if(sys=="rel"):
            ...
            lrel = Label(relroot, text="Enter initial Velocity in x, y & z direction in the boxes").grid(row=2, column=0,columnspan=5)
    ...
    def simulate():
        ...
        m=float(e2.get())
        menu.destroy()
    l2=Label(menu, text=" ").grid(row=5, column=0)
    b3 = Button(menu, text="Simulate!", command=simulate,width = 20).grid(row=6, column=0,columnspan = 2)
